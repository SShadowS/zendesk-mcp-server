...
// Singleton server instance for stateful mode
let singletonServer = null;

/**
 * Get singleton server instance (for stateful HTTP mode)
 * Creates it on first call, returns same instance after
 */
function getServer() {
  if (!singletonServer) {
...
/**
 * Initialize and return server instance
 * For stateful mode: returns singleton
 * For stateless mode: use createServer() directly
 */
async function initializeServer() {
  return getServer();
}

export { createServer, initializeServer, getServer };
